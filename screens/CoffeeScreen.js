...
export default class CoffeeScreen extends React.Component {
    ...
    constructor(props) {
        ...
        this.ref = firebase.firestore().collection('coffeeRec');
        this.coffee = null;
        this.state = {
            isLoading: true,
            coffee: [],
    ...
    componentDidMount() {
        this.coffee = this.ref.onSnapshot(this.onCollectionUpdate);
    }
            ...
            <View style={styles.container}>
                ...
                {this.state.isLoading ? <ActivityIndicator style={{ marginTop: 40 }} size="large" color='#c4463d' /> :
                    <ScrollView
                        ...
                        {
                            this.state.coffee.map((l, i) => (
                                <View key={i} style={{ paddingVertical: 15 }}>
                                    <View style={{ flexDirection: 'row' }}>
                                        <Text style={{ flex: 2, color: '#c4463d', fontWeight: '400', fontSize: 20 }}>{l.name}</Text>
